...
class DiverseBranchBlockPruner(tp.pruner.BasePruningFunc):
    ...
    def prune_in_channels(self, layer: DiverseBranchBlock, idxs: Sequence[int]):
        ...
        if hasattr(layer.dbb_1x1_kxk, 'idconv1'):
            layer.dbb_1x1_kxk.idconv1.id_tensor = self._prune_parameter_and_grad(layer.dbb_1x1_kxk.idconv1.id_tensor,
                                                                                 keep_idxs=keep_idxs, pruning_dim=1)
...
class DyHeadBlockPruner(tp.pruner.BasePruningFunc):
    def prune_in_channels(self, layer: DyHeadBlock_Prune, idxs: Sequence[int]):
        ...
        layer.spatial_conv_low.conv.weight = self._prune_parameter_and_grad(layer.spatial_conv_low.conv.weight,
                                                                            keep_idxs, 1)
        layer.spatial_conv_mid.conv.weight = self._prune_parameter_and_grad(layer.spatial_conv_mid.conv.weight,
                                                                            keep_idxs, 1)
        layer.spatial_conv_high.conv.weight = self._prune_parameter_and_grad(layer.spatial_conv_high.conv.weight,
                                                                             keep_idxs, 1)
